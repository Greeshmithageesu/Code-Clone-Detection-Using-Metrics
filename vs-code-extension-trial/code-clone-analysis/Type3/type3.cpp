    ...
    for(auto item: module){
        ...
        for (int i = 0; i < item.second.size(); i++){
            ...
            for(int j=0; j<item.second[i].second.size(); j++){
                ...
                // cout << item.second[i].second[j] << "88888888888888888888888888" << endl;
...
int getnVars(vector<pair<int, vector<string>>> &fun, int ind){
    ...
    for(int i=1; i<fun.size(); i++){
        ...
        bool isComma = true;
        for(int j=0; j<fun[i].second.size(); j++){
            // cout << fun[i].second[j] << "9999999999" << endl;
            if(check(fun[i].second[j])){
                isData = true;
            }
...
void printRow(metrics m){
    ...
    printf("%10d%10d%10d%10d%10d%10d%10d%10d%10d\n", m.nArgs, m.nVars, m.nConds, m.nLoops, m.nLines, m.nFunCalls, m.nReturn, m.nInputs, m.nOutputs);
}
...
vector<pair<string, string>> returnCandidatePairs(unordered_map<string, vector<pair<int, vector<string>>>> tempModule, unordered_map<string, vector<pair<int, vector<string>>>> tempTemplateCode){
    ...
    for(auto i: module1){
        ...
        while(j<i.second[0].second.size() && i.second[0].second[j] != ")"){
...
// cout << "HELLO !" << endl;
  ...
  vector<double> type3_match(len_candidatePairs);

  for(int i=0; i<candidatePairs.size(); i++){
    string fun1 = candidatePairs[i].first;
    string fun2 = candidatePairs[i].second;
    type3_match[i] = isClonePair(fun1, fun2, templateCode);
    cout << "Matching Percentage " << fun1 << " " << fun2 << " " << 100*isClonePair(fun1, fun2, module) << endl;
  }

    ofstream myfile(cwd + "/code-clone-analysis/Type3/co-ordinates.txt");
  ...
  if (myfile.is_open()){
    for (int i = 0; i < len_candidatePairs; i++){
      ...
      curr_line += ",";
      curr_line += to_string(type3_match[i]);
  ...
  string command = "python " + cwd + "/code-clone-analysis/Type3/step0.py " + cwd;
